...
(function () {
  ...
  const WHATSAPP_NUMBER = ""; // TODO: preencher com o número de WhatsApp (formato internacional, ex: 5511999999999)
  const FORM_ENDPOINT = "contact.php"; // script PHP incluído nesta mesma pasta — funciona ao publicar na Hostinger
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      const hasEndpoint = FORM_ENDPOINT && FORM_ENDPOINT.trim().length > 0;

      if (!hasEndpoint) {
        formStatus.removeAttribute('data-state');
        formStatus.textContent = 'Formulário pronto para integração. Configure o endpoint de contato.';
        return;
      }
      ...
      fetch(FORM_ENDPOINT, {
        ...
        .then(function (response) {
          return response.json().catch(function () {
            // resposta não é JSON válido (ex: endpoint mal configurado)
        ...
        .then(function (data) {
          if (data && data.success) {
            ...
            formStatus.setAttribute('data-state', 'error');
            formStatus.textContent = (data && data.message) || 'Não foi possível enviar agora. Tente novamente em instantes.';
          }
        })
        .catch(function () {
          formStatus.setAttribute('data-state', 'error');
